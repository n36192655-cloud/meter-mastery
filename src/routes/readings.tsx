...
import { addPending, syncPending } from "@/lib/sync";
...
function ReadingsPage() {
  ...
  // Realtime subscriptions on readings + bills for this tenant
  useEffect(() => {
    if (!tenantId) return;
  ...
  function handleOcr(res: OcrResult) {
    ...
    // Convert dataURL → Blob (binary) for storage upload
    ...
    <div className="space-y-6">
      ...
      <div className="flex gap-2 flex-wrap">
        ...
        <Button size="sm" variant={tab === "bills" ? "default" : "outline"} onClick={() => setTab("bills")}>
          الفواتير الحية {bills.length > 0 && <Badge className="ms-1" variant="secondary">{bills.length}</Badge>}
        </Button>
      </div>
      ...
      {tab === "bills" && (
        <Card>
          <CardHeader><CardTitle>الفواتير الحية ({bills.length})</CardTitle></CardHeader>
          <CardContent className="p-4 overflow-auto">
            <Table>
              <TableHeader>
                <TableRow>
